...
func NewService(uploadDir string) (*Service, error) {
	if uploadDir == "" {
		uploadDir = "uploads"
	}
	...
	return &Service{uploadDir: uploadDir}, nil
}
...
func (s *Service) Save(category string, file multipart.File, originalName string) (string, error) {
	...
	if _, ok := allowedExtensions[ext]; !ok {
		return "", fmt.Errorf("format de fichier non supporté (%s). Utilisez .xls ou .xlsx", ext)
	}
	...
	if err != nil {
		return "", fmt.Errorf("impossible de créer le fichier %s: %w", finalName, err)
	}
	defer dst.Close()

	if _, err := io.Copy(dst, file); err != nil {
		return "", fmt.Errorf("échec lors de l'enregistrement de %s: %w", finalName, err)
	}
